...
then
    ...
    then
        cd news
        export DJANGO_SETTINGS_MODULE="${DJANGO_SETTINGS_MODULE:="news.settings"}"
        if pytest --tb=line 1>&2;
        then
            cd ../note
            unset DJANGO_SETTINGS_MODULE
            export DJANGO_SETTINGS_MODULE="${DJANGO_SETTINGS_MODULE:="note.settings"}"
            if pytest --tb=line 1>&2;
            then
                exit 0
            else
                status=$?
                print_message " При запуске упали тесты для проекта Note. Проверь тесты этого проекта " "=" 1
                echo \`\`\` 1>&2
                exit $status
            fi
        else
            status=$?
            print_message " При запуске упали тесты для проекта News. Проверь тесты этого проекта " "=" 1
            echo \`\`\` 1>&2
            exit $status
        fi
    else
        status=$?
        print_message " Тестов нет " "=" 1
        echo \`\`\` 1>&2
        exit $status
    fi
else
